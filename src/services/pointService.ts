...
export const createPoint = async (
  ...
  try {
    ...
    console.log('📍 Création point pour utilisateur:', currentUser.uid);
    ...
    return { 
      success: true, 
      pointId,
      point: { ...point, id: pointId, userId: currentUser.uid }
    };
...
export const getMyPoints = async (): Promise<{ 
  ...
  try {
    ...
    console.log('📥 Chargement des points pour utilisateur:', currentUser.uid);
    
    // 1. Récupérer tous les user_points et filtrer localement
    const userPointSnapshot = await getDocs(collections.user_point);
    const myUserPoints = userPointSnapshot.docs
      .filter(doc => doc.data().id_user === currentUser.uid)
